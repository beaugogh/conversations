...
def i_am_human(browser):
    browser.get('https://www.perplexity.ai/')
    sleep(20)
    browser.switch_to.frame(browser.find_element(By.TAG_NAME, 'iframe'))
    browser.find_element(By.CLASS_NAME, 'ctp-checkbox-label').click()
            ...
            try:
                for i in range(start_ind, min(end_ind, len(df))):
                    try:
                        ...
                        f.write(line + '\n')
                        f.close()
                    except AssertionError: # skip assertion errors
                        continue
                break # break the loop until all questions are scraped
            except Exception as e: # resume other errors
                print(e)
                cooldown = 300
